...
public class IMT2019025 {
    ...
    static final String PASS = "Manumanohar@74";
    ...
    public static int get_cust_by_pno(String pno)
    {
        String sql;
        try
        {
            sql = String.format("select * from customer where phone_number = '%s'", pno);
            ResultSet rs = stmt.executeQuery(sql);
            while(rs.next())
            {
                System.out.println("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~");
